...
def refresh_if_stale():
    """Fetch fresh data if the cache is older than POLL_SECONDS.

    This runs synchronously inside the request that needs it, rather than
    a background thread, so it always executes in the same process that's
    serving the request — sidestepping any preload/fork weirdness where a
    background thread's updates never reach the worker process.
    """
    with _lock:
        age = time.time() - _state["last_fetch_attempt"]
        should_fetch = age >= POLL_SECONDS or _state["last_updated"] is None
        if should_fetch:
